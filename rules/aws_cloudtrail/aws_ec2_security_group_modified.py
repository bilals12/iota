...
def rule(event):
    ...
    if not is_successful(event) or event.get("eventName") != "AuthorizeSecurityGroupIngress":
        return False

    # Check for 0.0.0.0/0 CIDR blocks
    ip_permissions = deep_get(event, "requestParameters", "ipPermissions", "items", default=[])
    for permission in ip_permissions:
        ...
        for ip_range in ip_ranges:
            if ip_range.get("cidrIp") == "0.0.0.0/0":
                ...
                if from_port in [22, 3389, 3306, 5432, 1433, 27017]:  # SSH, RDP, databases
                    return True
