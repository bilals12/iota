...
Everything is driven by **[`.github/workflows/release.yml`](.github/workflows/release.yml)** (“Release pipeline”):

- **Push to `main`**: `scripts/next-release-version.sh` computes the next tag from commits since the latest `v*.*.*` tag (see [`docs/breaking-changes.md`](docs/breaking-changes.md)). The workflow pushes an annotated tag using the **`RELEASE_PLEASE_TOKEN`** secret (PAT with **contents: write**). GitHub does not trigger downstream workflows from tag pushes made with `GITHUB_TOKEN`, so this PAT is required.
- **Tag `v*.*.*` or “Run workflow”** with an existing tag: build Linux binaries, attach them to the GitHub Release, build and push a **single multi-arch** Docker image (`linux/amd64` + `linux/arm64`) to Docker Hub (`bilals12/iota`), optionally bump [`iota-deployments`](https://github.com/bilals12/iota-deployments), and sync **`.release-please-manifest.json`** on `main` (commit message contains `[skip release]` so another tag is not cut from that push).
...
See **[`docs/breaking-changes.md`](docs/breaking-changes.md)** for the full rubric. Summary:

- **`feat:`** → **major**
- **`fix:`**, **`refactor:`**, **`perf:`** → **minor**
- **`style:`**, **`test:`**, **`docs:`**, **`build:`**, **`ops:`**, **`chore:`**, and non-conventional messages → **patch**
- **`BREAKING CHANGE:`** / **`BREAKING-CHANGE:`** in the body, or **`!`** after the type (e.g. `feat!:`) → **major**

Use **`[skip release]`** in a commit message to push to `main` without creating a release tag from that push.

## Troubleshooting tag → publish

If **binaries or Docker images never appear** after a tag exists, the publish job may not have run because the tag was pushed with **`GITHUB_TOKEN`**. Use **`RELEASE_PLEASE_TOKEN`** for the tag step, or run **Actions → Release pipeline → Run workflow** with the tag to backfill assets.
